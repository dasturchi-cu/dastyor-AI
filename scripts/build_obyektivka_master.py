...
from features.obyektivka.docx_zip import clone_with_replacements, count_page_breaks
...
def _party_military_replacements() -> list[tuple[str, str]]:
    return [
        ("{{millati}}\tйўқ", "{{millati}}\t{{partiyaviyligi}}"),
        ("{{chet_tillari}}\tйўқ", "{{chet_tillari}}\t{{harbiy_unvoni}}"),
    ]
...
def main() -> None:
    ...
    replacements = list(TEXT_REPLACEMENTS)
    replacements.extend(_party_military_replacements())
  ...
  # relatives title name
    parts_replacements = [("Эшматов Ботир Баҳодировичнинг", "{{fish}}нинг")]
    from features.obyektivka.docx_zip import read_parts, write_parts, replace_text_in_xml

    parts = read_parts(OUT)
    xml = parts["word/document.xml"].decode("utf-8")
    xml = replace_text_in_xml(xml, parts_replacements)
